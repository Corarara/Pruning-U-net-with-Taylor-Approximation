...
def batch(iterable, batch_size, shuffle=True): # 需要改成False
    """
    Yields lists by batch
    
    在每个batch里做permutation
    """
    b = []
    for i, t in enumerate(iterable): # i is the count (0,1,...), t is the element in iterable
        b.append(t)
        if (i + 1) % batch_size == 0:
            if shuffle:
                random.shuffle(b)
    ...
    if len(b) > 0: # 当data size 不被batch_size 整除时，加上多余shuffle之后的部分
        if shuffle:
            random.shuffle(b)
        yield b
